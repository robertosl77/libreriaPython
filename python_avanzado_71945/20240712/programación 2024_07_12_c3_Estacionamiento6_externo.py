...
import json
def leer_json():
    with open(f"estacionamiento.json",  mode="r"  ,encoding='utf-8') as objeto_json:
       info_grabada_a_programa = json.load(objeto_json )
    return info_grabada_a_programa
#########################################################################################################################
def grabar_json (info_a_grabar):
    with open(f"estacionamiento.json",   mode= "w",encoding='utf-8') as obj_json:
        json.dump(info_a_grabar, obj_json ,ensure_ascii=False, indent=4)

# ~ grabar_json(dic_patentes)
# ~ exit()
#---------------------------------------------------------------------
